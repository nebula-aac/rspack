---
description: '顶级的 output 包含一组选项，指导 Rspack 如何以及在哪里输出 bundles、assets，以及使用 Rspack 打包或加载的任何其他内容。'
---
...
Asset module 输出的文件名称。这个值可以被 [rules[].generator.filename](/config/module-rules#rulesgenerator) 覆盖。

:::info Asset module 作为一个单独的文件输出的情况

- 模块类型为 `'asset'` 且 asset 满足 [`rules[].parser.dataUrlCondition`](/config/module-parser#assetdataurlcondition) 设定值
...
- **类型：** `string | ((pathData: PathData, assetInfo?: AssetInfo) => string)`
- **默认值：** 当 [`output.filename`](#outputfilename) 为字符串时，根据其推断；否则为 `'[id].js'`。
...
未配置此选项时，Rspack 根据 [`output.filename`](#outputfilename) 推断默认值：

- 如果 `output.filename` 是包含 `[name]`、`[id]`、`[chunkhash]` 或 `[contenthash]` 的字符串，则直接复用该字符串。其中 `[name]` 不会被替换成 `[id]`。
- 否则，Rspack 会在文件名前添加 `[id].`，保留目录和查询字符串。
- 如果 `output.filename` 是函数，则默认为 `'[id].js'`。Rspack 不会调用该函数来推断 `output.chunkFilename`。

| `output.filename`     | 默认的 `output.chunkFilename` |
| --------------------- | ----------------------------- |
| `'[name].js'`         | `'[name].js'`                 |
| `'[name].mjs'`        | `'[name].mjs'`                |
...
:::tip
查看 [文件名占位符](/config/filename-placeholders) 了解更多信息。
:::
...
:::tip
这个选项的默认值取决于 [`target`](/config/target) 和 [`output.chunkFormat`](#outputchunkformat) 设置。要了解更多详情，请在 Rspack [默认设置中](https://github.com/web-infra-dev/rspack/blob/main/packages/rspack/src/config/defaults.ts)搜索 `"chunkLoading"`。
:::
...
当 `output.chunkFilename` 为字符串时，Rspack 会将其中的 `.js`、`.mjs` 或 `.cjs` 扩展名替换为 `.css`，并保留查询字符串。当其为函数时，默认值为 `'[id].css'`，不会调用该函数来推断 CSS 文件名。
...
- **类型：** `string | ((pathData: PathData, assetInfo?: AssetInfo) => string)`
- **默认值：** 当 [`output.filename`](#outputfilename) 为字符串时，根据其推断；否则为 `'[id].css'`。

此选项决定了 CSS 文件的名称，参阅 [`output.filename`](/config/output#outputfilename)，了解可能的值。

当 `output.filename` 为字符串时，Rspack 会将其中的 `.js`、`.mjs` 或 `.cjs` 扩展名替换为 `.css`，并保留查询字符串。如果不包含这些扩展名，则保持字符串不变。

| `output.filename`        | 默认的 `output.cssFilename` |
| ------------------------ | --------------------------- |
| `'scripts/[name].js'`    | `'scripts/[name].css'`      |
| `'[name].mjs?version=1'` | `'[name].css?version=1'`    |
| 函数                     | `'[id].css'`                |

使用函数配置 JavaScript 文件名时，如果需要按名称输出 CSS 产物，可以单独配置 CSS 文件名：
...
此选项仅在 [`devtool`](/config/devtool) 使用了需要模块名称的选项时使用。
...
| [relative-resource-path] | 模块资源相对于 source map 文件所在目录的相对路径；对于内联 source map，则相对于产物文件所在目录 |
...
`output.environment` 的默认值由 [target](/config/target) 自动推断。

Rspack 会基于 `target` 识别目标环境支持的语言能力，例如：

- 若目标浏览器已支持箭头函数，Rspack 会默认使用箭头函数以缩短 runtime 代码
- 若 `target` 指向特定 Node 版本，Rspack 会根据该版本的语法能力推断对应字段
...
要使用的 hash 摘要的长度，详见 [Hash 长度](/config/filename-placeholders#hash-长度)。
...
- **类型：** `string`
- **默认值：** 当 [`output.module`](#outputmodule) 为 `true` 时，为 `'[id].[fullhash].hot-update.mjs'`，否则为 `'[id].[fullhash].hot-update.js'`。
...
- **类型：** `string`
- **默认值：** `"rspackHotUpdate" + output.uniqueName`
...
- **类型：** `string`
- **默认值：** 当 [`output.module`](#outputmodule) 为 `true` 时，为 `'[runtime].[fullhash].hot-update.json.mjs'`，否则为 `'[runtime].[fullhash].hot-update.json'`。
...
- **类型：** `boolean`
- **默认值：** 当 [`output.module`](#outputmodule) 为 `true` 时为 `false`，否则为 `true`。
...
- **类型：** `string`
  ...
  类型默认包括 `'var'`、`'module'`、`'modern-module'`、`'assign'`、`'assign-properties'`、`'this'`、`'window'`、`'self'`、`'global'`、`'commonjs'`、`'commonjs2'`、`'commonjs-module'`、`'commonjs-static'`、`'amd'`、`'amd-require'`、`'umd'`、`'umd2'`、`'jsonp'` 和 `'system'`，除此之外也可以通过插件添加。
...
这是一条更偏向兼容性的 ESM library 路径：Rspack 仍沿用常规的 chunk / runtime 渲染流程，再把入口导出转换成 `export` 语句。它更适合较简单的 ESM 库输出，或者你只需要 webpack 风格 `module` 语义的场景。
...
这项配置会启用 Rspack 为 ESM 库单独设计的渲染路径，直接输出 `import` / `export`，并支持库场景下的 tree-shaking、scope hoisting、code splitting 以及 [`output.library.preserveModules`](#outputlibrarypreservemodules)。

:::tip `module` 和 `modern-module` 的区别

- `module` 仍走常规的 chunk / runtime 渲染流程，属于更接近 webpack 兼容语义的 ESM 输出。
- `modern-module` 会由专用的 ESM library 插件接管 chunk 渲染、链接、scope hoisting 和 splitChunks，更适合发布可 tree-shake、可 code-split 的 ESM 库。
- `modern-module` 当前是为补齐库场景 ESM 能力而单独引入的类型；等能力进一步稳定后，计划与 `module` 收敛为同一个 `library.type`。
- `preserveModules` 仅在 `modern-module` 下可用。
- 当你配置了 `output.library` 且 `output.module = true` 时，如果没有显式指定 `output.library.type`，Rspack 当前默认会使用 `modern-module`。
- 不允许在同一次编译中把 `modern-module` 和其他 `library.type` 混用，因为它会同时影响 chunk loading、chunk 渲染和 scope hoisting 的行为。
...
:::tip 如何选择 CommonJS 的 `library.type`

| 类型              | 写入目标            | 更适合的场景                                           |
| ----------------- | ------------------- | ------------------------------------------------------ |
| `commonjs`        | `exports[...]`      | 兼容已有依赖 `exports` 对象的历史配置；更偏兼容写法    |
| `commonjs2`       | `module.exports`    | 通用的 Node.js / CJS 库输出；大多数新配置优先选它      |
| `commonjs-module` | `module.exports`    | 当前与 `commonjs2` 等价，主要用于兼容历史 webpack 配置 |
| `commonjs-static` | `exports.foo = ...` | 需要让 Node.js ESM 能够对 CJS 产物做具名导入           |

Rspack 当前保留这些写法主要是为了兼容 webpack 生态，并没有在配置层面对它们做废弃。对于新项目，通常优先考虑 `commonjs2` 或 `commonjs-static`；只有在迁移既有配置、或明确依赖 `exports` 赋值语义时，再使用 `commonjs` / `commonjs-module`。
...
请注意，根据[对象赋值部分](#通过对象赋值暴露)，省略 `library.name` 将导致入口起点返回的所有属性直接赋值给根对象。示例：
...
:::info
仅在 `library.type` 为 `modern-module` 时生效。
:::

当启用后，Rspack 会保留传入目录下模块的原始文件结构。每个模块会被输出为单独的文件，保持与源码相同的相对路径。

`preserveModules` 是一种特殊的打包形式——可以将其理解为 **bundle + splitChunks，其中每个模块会被拆分为单独的 chunk**。与 bundleless 工具不同，Rspack 仍然会执行完整的打包流程（tree shaking、scope hoisting、代码压缩等），只是将输出拆分为每个模块对应一个输出文件。

:::warning
**Barrel 文件可能会丢失导出。** 由于 `preserveModules` 仍然是一种打包形式，只有 entry 的导出会被保留。如果 barrel 文件（例如从其他模块重新导出的 `index.ts`）不是 entry，其重新导出的内容不会被保留在输出中。如果确实需要严格暴露导出，请将对应模块设为 entry。
:::
...
- `true`: 启用基础的注释输出，包括模块路径信息。
- `false`: 不输出任何注释，这是默认行为。
- `'verbose'`：输出更详细的信息，例如模块导出、运行时信息、tree shaking 信息和 `bailout` 原因。适合排查构建问题或深入分析构建结果。
...
可以使用 [`output.filename`](#outputfilename) 中的 `[name]`、`[id]`、`[fullhash]` 和 `[chunkhash]` 替换符。此外，还可以使用 [文件名占位符](/config/filename-placeholders) 中在文件级别列出的替换符。
...
<Attribution url="https://webpack.docschina.org/configuration/output/" />
